...
For example, we use GNU coreutil dataset to train the LSTM regression model. The script in `get started` does three work. First, it trains the model, secondly, it evaluates the model, thirdly it runs a simulation for program `arch` with the model you used.
...
+ --tree_for_assert, true for using abstract trees as tree nodes in tree-LSTM model
 ...
 For tree-LSTM, our model uses the feature vector instead of the abstract tree by default, which makes it functionally works like LSTM. But you may use `--tree_for_assert` option to use the abstract tree. To make it more practicable, we make some inductions, which replace oversized abstract trees into vectors. Our results suggest the improvement does not worth the cost. If you want to further research the tree structure, you could work on it in `preprocessing/abstract_tree_extraction.py`.
...
We support two main settings including KNN implemented in sklearn(non-adaptive so poor in performance, better in efficiency, mainly for comparison), incremental-KNN. You may use different command-line arguments. To use our predictor as a tool, you can use the `KNN_Predictor.py` from the home directory. 
...
To use our predictor as a tool, you can use the `KNN_Predictor.py` from the home directory. Taking `angr` with `z3` solver backend as an example, you need to replace the solving `result = solver.check()` like:
```
